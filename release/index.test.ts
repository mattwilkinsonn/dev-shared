...
import { actionsUrl, runOnce, validateVersion, verifyBumps } from "./index.ts";
...
describe("verifyBumps", () => {
	...
	test("returns no errors when the bump took", () => {
		expect(verifyBumps(cargoOk, "0.4.0")).toEqual([]);
	});

	test("flags a stale Cargo.toml and echoes its version lines", () => {
		const cargoStale = 'version = "0.3.7"\nedition = "2024"\n';
		expect(verifyBumps(cargoStale, "0.4.0")).toEqual([
...
describe("actionsUrl", () => {
	...
	test("derives the workflow URL from a github repository field", async () => {
		const cargo =
			'[package]\nname = "jj-hooks"\nrepository = "https://github.com/mattwilkinsonn/jj-hooks"\n';
		expect(await actionsUrl(async () => cargo)).toBe(
			"   https://github.com/mattwilkinsonn/jj-hooks/actions/workflows/release.yml",
		);
	});

	test("strips a trailing .git from the repository URL", async () => {
		const cargo =
			'repository = "https://github.com/mattwilkinsonn/jj-gt.git"\n';
		expect(await actionsUrl(async () => cargo)).toBe(
			"   https://github.com/mattwilkinsonn/jj-gt/actions/workflows/release.yml",
		);
	});

	test("falls back when the repository field is missing", async () => {
		expect(await actionsUrl(async () => '[package]\nname = "x"\n')).toBe(
			generic,
		);
	});

	test("falls back when the repository is not a github URL", async () => {
		const cargo = 'repository = "https://gitlab.com/x/y"\n';
		expect(await actionsUrl(async () => cargo)).toBe(generic);
	});

	test("falls back when Cargo.toml is unreadable", async () => {
		expect(
			await actionsUrl(async () => {
...
describe("runOnce guards", () => {
	test("bad version exits 1 before any command runs", async () => {
		...
		expect(errs).toEqual([
			"usage: release vX.Y.Z (or vX.Y.Z-rc.1); got: 'nope'",
		]);
	});
...
describe("runOnce happy path", () => {
	test("runs every step in the exact bash order", async () => {
		const { deps, commands, logs } = makeDeps({
			responses: HAPPY_RESPONSES,
			files: { ...HAPPY_FILES },
		});
		...
		// Final success echo carries the derived actions URL.
